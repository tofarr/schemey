...
T = TypeVar('T')
_SchemaFactoryABC = 'old.factory.schema_factory_abc.SchemaFactoryABC'
_SchemaABC = 'old.schema_abc.SchemaABC'
    ...
    def get_schema(self, type_: Type[T], default_value: Optional[T] = None) -> _SchemaABC:
        ...
        if not schema:
            ...
            for factory in self._factories:
                schema = factory.create(resolved_type, self, default_value)
                if schema:
                    break
            if not schema:
