...
class ObjectSchemaLoader(SchemaLoaderABC):
    ...
    def load(self, item: ExternalItemType, json_context: JsonSchemaContext) -> Optional[SchemaABC]:
        ...
        properties = {k: self._load_optional(k, s, required, json_context) for k, s in properties.items()}
        loaded = ObjectSchema(
            name=name,
    ...
    @staticmethod
    def _load_optional(key: str, item: ExternalItemType, required: Set[str], context: JsonSchemaContext) -> SchemaABC:
