...
def simplify_kwargs(schema):
    ...
    if schema.minimum is not None and schema.exclusive_minimum is not None:
        del kwargs['minimum' if schema.minimum <= schema.exclusive_minimum else 'exclusive_minimum']
    if schema.maximum is not None and schema.exclusive_maximum is not None:
        del kwargs['maximum' if schema.maximum >= schema.exclusive_maximum else 'exclusive_maximum']
    return kwargs
