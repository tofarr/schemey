...
class TestNumberSchema(TestCase):
    ...
    def test_get_schema_errors(self):
        ...
        self.assertEqual(
            list(schema.get_schema_errors("foo", [])), [SchemaError("", "type", "foo")]
        )
        self.assertEqual(
            list(schema.get_schema_errors(False, ["foo", "bar"])),
            [SchemaError("foo/bar", "type", False)],
        )
    ...
    def test_simplify(self):
        self.assertEqual(
            dict(type="number", exclusiveMinimum=5),
        ...
        self.assertEqual(
            dict(type="number", minimum=6),
            dump(NumberSchema(minimum=6, exclusive_minimum=5).simplify()),
        )
        self.assertEqual(
            dict(type="number", exclusiveMaximum=5),
