...
from schemey.schema_context import schema_for_type, new_default_schema_context
...
class TestObjectSchema(TestCase):
    ...
    def test_to_json_schema(self):
        schema = schema_for_type(ExternalItemType).to_json_schema()
        assert schema == dict(type='string', format='json_object')
        schema = schema_for_type(ExternalType).to_json_schema()
