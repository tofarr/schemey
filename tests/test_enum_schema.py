...
class TestEnumSchema(TestCase):
    ...
    def test_get_schema_errors(self):
        ...
        self.assertEqual(
            [SchemaError("", "type", "pop")], list(schema.get_schema_errors("pop"))
        )
        self.assertEqual(
            [SchemaError("a/b", "type", "pop")],
            list(schema.get_schema_errors("pop", ["a", "b"])),
        )
        s = schema.json_schema
        self.assertEqual(
            [SchemaError("a/b", "value_not_permitted", 1)],
            list(s.get_schema_errors(1, ["a", "b"])),
        )
    ...
    def test_dump_and_load(self):
        schema = EnumSchema(
            name="foobar", enum={"foo", "bar", "zap", "bang"}, description="Foobar"
        )
        dumped = dump(schema)
        expected_dump = dict(
            name="foobar", enum=list(schema.enum), description="Foobar"
        )
    ...
    def test_get_normalized_type(self):
        ...
        # noinspection PyTypeChecker
        self.assertEqual(
            {"foo", "bar", "zap", "bang"}, set(s.name for s in standard_type)
        )
        # noinspection PyTypeChecker
        self.assertEqual(
            {"foo", "bar", "zap", "bang"}, set(s.value for s in standard_type)
        )
        self.assertIs(
            standard_type, schema.get_normalized_type(existing_types, dataclass)
        )
